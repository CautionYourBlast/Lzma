...
HRESULT ReadStream(ISequentialInStream *stream, void *data, size_t *processedSize) throw()
{
  ...
  *processedSize = 0;
  ...
  {
#ifdef CLZMA
    const UInt32 curSize = (size < lzma_wrp_stream_read_size) ? (UInt32)size : lzma_wrp_stream_read_size;
#else
...
HRESULT WriteStream(ISequentialOutStream *stream, const void *data, size_t size) throw()
{
  ...
  {
#ifdef CLZMA
    const UInt32 curSize = (size < lzma_wrp_stream_write_size) ? (UInt32)size : lzma_wrp_stream_write_size;
#else
    UInt32 curSize = (size < kBlockSize) ? (UInt32)size : kBlockSize;
#endif
    UInt32 processedSizeLoc;
